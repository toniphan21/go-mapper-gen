...
### Text for nullable char, varchar and text

when you have a nullable `char`, `varchar` or `text` column in your database, `sqlc` will
generate type `pgtype.Type`, the golang equivalent type is `*string`.
...
[//]: # (EmitCode:examples/sqlc/pgtype/09-text)
